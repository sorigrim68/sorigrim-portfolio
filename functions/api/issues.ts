...
export const onRequestGet: PagesFunction<Env> = async ({ env, request }) => {
  ...
  const row = await env.DB.prepare(
    'SELECT data, updated_at FROM xconda_issues WHERE share_key = ?',
  ).bind(key).first<IssueRow>()

  if (!row) {
    const data = JSON.stringify({ items: [] })
    await env.DB.prepare(
      'INSERT INTO xconda_issues (share_key, data, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)',
    ).bind(key, data).run()
...
export const onRequestPut: PagesFunction<Env> = async ({ env, request }) => {
  ...
  const current = await env.DB.prepare(
    'SELECT updated_at FROM xconda_issues WHERE share_key = ?',
  ...
  await env.DB.prepare(`INSERT INTO xconda_issues (share_key, data, updated_at)
